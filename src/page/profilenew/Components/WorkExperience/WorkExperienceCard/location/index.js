...
class LocationReact extends React.Component {
  ...
  localChinaCodeToDisplay(prevState) {
    return `${
      prevState.country.countryList[prevState.country.countrySelectedIndex].name
    } ${
      prevState.china.provinceList[prevState.china.provinceSelectedIndex].name
    } ${prevState.china.cityList[prevState.china.citySelectedIndex].name} ${
      prevState.china.districtList[prevState.china.districtSelectedIndex].name
    }`.replace(new RegExp(` ${this.text.unselected}`, 'g'), '');
  }

  locate() {
    return {
      countryCode: this.state.country.countryList[
        this.state.country.countrySelectedIndex
      ].id,
  ...
  handleCountryChange(data) {
    ...
    if (
      selectedId ===
      this.state.country.countryList[this.state.country.countrySelectedIndex].id
    ) {
      return;
    }
    if (selectedId === 'CHN') {
      ...
      get('/static/location?target=1&id=000000').then(data => {
        ...
        let provinceSelectedIndex = LocationReact.setSelected(
          this.defaultProvinceId,
          provinceList
        );
        // city
        let cityList = [];
        this.addUnknownItem(cityList);
        let citySelectedIndex = LocationReact.setSelected(
          this.unknownId,
          cityList
        );
        // district
        ...
        let districtSelectedIndex = LocationReact.setSelected(
          this.unknownId,
          districtList
        );
        //
        this.setState(prevState => {
          return {
            country: Object.assign({}, prevState.country, {
              countrySelectedIndex: Location.idToindex(
                selectedId,
                prevState.country.countryList
              ),
            }),
      ...
      this.setState(prevState => {
        return {
          display:
            prevState.country.countryList[
              prevState.country.countrySelectedIndex
            ].name,
        };
      }, this.ready);
    }
  ...
  handleProvinceChange(data, force = false) {
    ...
    if (
      !force &&
      selectedId ===
        this.state.china.provinceList[this.state.china.provinceSelectedIndex].id
    ) {
      return;
    }
    get(`/static/location?target=2&id=${selectedId}`).then(data => {
      ...
      this.setState(prevState => {
        return {
          code:
            selectedId === this.unknownId
              ? prevState.china.cityList[prevState.china.citySelectedIndex].id
              : selectedId,
        };
      });
      this.setState(prevState => {
    ...
    this.setState(prevState => {
      return {
        china: Object.assign({}, prevState.china, {
          provinceSelectedIndex: Location.idToindex(
            selectedId,
            prevState.china.provinceList
          ),
          cityList: null,
          citySelectedIndex: 0,
          districtList: null,
  ...
  handleCityChange(data) {
    ...
    if (
      selectedId ===
      this.state.china.cityList[this.state.china.citySelectedIndex].id
    ) {
      return;
    }
    ...
    this.setState(prevState => {
      return {
        china: Object.assign({}, prevState.china, {
          citySelectedIndex: Location.idToindex(
            selectedId,
            prevState.china.cityList
          ),
          districtList: null,
          districtSelectedIndex: 0,
        }),
  ...
  handleDistrictChange(data) {
    ...
    if (
      selectedId ===
      this.state.china.districtList[this.state.china.districtSelectedIndex].id
    ) {
      return;
    }
    // READY
    this.setState(prevState => {
      return {
        china: Object.assign({}, prevState.china, {
          districtSelectedIndex: Location.idToindex(
            selectedId,
            prevState.china.districtList
          ),
        }),
      };
    });
  ...
  async componentDidMount() {
    ...
    if (this.state.code === '999999') {
      // OTH
      country.countrySelectedIndex = LocationReact.setSelected(
        'OTH',
        country.countryList
      );
      // READY
      ...
      // #3
      china.provinceSelectedIndex = LocationReact.setSelected(
        provinceId,
        china.provinceList
      );
      // city
      ...
      // #3
      china.citySelectedIndex = LocationReact.setSelected(
        cityId,
        china.cityList
      );
      // district
      ...
      // #3
      china.districtSelectedIndex = LocationReact.setSelected(
        districtId,
        china.districtList
      );
      // READY
      this.setState(
        {
          render: 1,
          display: `${country.countryList[country.countrySelectedIndex].name} ${
            china.provinceList[china.provinceSelectedIndex].name
          } ${china.cityList[china.citySelectedIndex].name} ${
            china.districtList[china.districtSelectedIndex].name
          }`.replace(new RegExp(` ${this.text.unselected}`, 'g'), ''),
          country: country,
          china: china,
        },
    ...
    switch (this.state.render) {
      ...
      case 1:
        return this.props.edit ? (
          <div>
            {/* edit */}
            <MDBSelect getValue={this.handleCountryChange}>
            ...
            {this.state.china ? (
              <div>
                <MDBSelect getValue={this.handleProvinceChange}>
                  <MDBSelectInput selected='' />
                  <MDBSelectOptions>
                ...
                {/* {console.log(this.state.china.cityList)} */}
                {this.state.china.cityList ? (
                  <MDBSelect getValue={this.handleCityChange}>
                    <MDBSelectInput selected='' />
                    <MDBSelectOptions>
                ...
                {/* {this.state.china.districtList ? (
                  <MDBSelect getValue={this.handleDistrictChange}>
                    <MDBSelectInput selected='' />
                    <MDBSelectOptions>
          ...
          <div>
            {/* display */}
            <span className={this.props.className}>{this.state.display}</span>
          </div>
        );
      default:
        return null;
